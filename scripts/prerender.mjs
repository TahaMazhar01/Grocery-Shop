...
await page('/',config.tagline,`The things you love. The staples you need. Explore ${config.brand}.`,`<img src="${esc(config.hero.image)}" alt="A paper grocery bag of fresh produce and bread" width="768" height="512"/><nav>${catalog.categories.map(c=>`<a href="/category/${esc(c.id)}">${esc(c.name)}</a> `).join('')}</nav>${listing(active.filter(p=>p.featured))}`);
